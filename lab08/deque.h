...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        block_pointers = new Item * [bp_array_size];
        block_pointers_end = block_pointers + bp_array_size - 1;
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                if (*source.first_bp == source.block_pointers[bp_array_index]) first_bp = block_pointers + bp_array_index;
                
                //If this is the back_ptr of source, then set the back_ptr of this deque
                // STUDENT WORK...
                
                if (*source.last_bp == source.block_pointers[bp_array_index]) last_bp = block_pointers + bp_array_index;
                ...
                for (size_type block_item_index = 0; block_item_index < block_size; ++block_item_index)
                {
                    ...
                    // set front ptr
                    if (source.front_ptr /***/ == source.block_pointers[bp_array_index] + block_item_index) front_ptr = block_pointers[bp_array_index] + block_item_index;

                    // set back ptr
                    if (source.back_ptr /***/ == source.block_pointers[bp_array_index] + block_item_index) back_ptr = block_pointers[bp_array_index] + block_item_index;

                    // copy each item from each block
    ...
    void deque<Item>::push_front(const value_type& entry)
    {
        ...
        else if ((*first_bp == front_ptr) && (first_bp == block_pointers))
        {
            ...
            reserve(); // increase block size by 20
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        else if ((back_ptr == ((*last_bp) + (block_size - 1))) && (last_bp == block_pointers_end))
        {
            ...
            last_bp--;
            *last_bp = new Item[block_size];
            back_ptr = *first_bp; //+block_size - 1;
            *back_ptr = entry;
        }
    }
    ...
    void deque<Item>::pop_front()
    {
        ...
        else if (front_ptr == ((*first_bp) + block_size - 1))
        {
            ...
            // must make pointers NULL before delete or " Error in `./a': double free or corruption (fasttop): 0x0000000002119120"
            *first_bp = NULL;
            delete[] *first_bp;
            first_bp++;
            front_ptr = *first_bp;
